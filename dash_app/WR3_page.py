...
def CBUnits(barometricpressure):
    ...
    if (English_Metric == False):  # english units
        barometricpressure = barometricpressure * 0.2953/10.00
...
def generateCurrentWeatherJSON():
    try:
        ...
        cur = con.cursor()
        query = "SELECT * FROM `WeatherRack3` ORDER BY id DESC LIMIT 1"
        # print("query=", query)
        cur.execute(query)
        ...
        # get column names
        query = "SHOW COLUMNS FROM WeatherRack3"
        cur.execute(query)
        names = cur.fetchall()
        ...
        # calendar day rain
        query = "SELECT id, TotalRain, TimeStamp FROM WeatherRack3 WHERE DATE(TimeStamp) = CURDATE() ORDER by id ASC"
        cur.execute(query)
        ...
        # Calendar Month
        query = "SELECT id, TotalRain, TimeStamp FROM WeatherRack3 WHERE MONTH(TimeStamp) = MONTH(NOW()) AND YEAR(TimeStamp) = YEAR(NOW())"
        cur.execute(query)
        rainspanrecords = cur.fetchall()
        ...
        query = "SELECT id, TotalRain, TimeStamp FROM WeatherRack3 WHERE (TimeStamp > '%s') ORDER BY TimeStamp " % (
            before)
...
def fetchWindData(timeDelta):
    try:
        ...
        before = now - timeDelta
        before = before.strftime('%Y-%m-%d %H:%M:%S')
        query = "SELECT WindSpeed, WindDirection FROM WeatherRack3 WHERE (TimeStamp > '%s') ORDER BY TimeStamp " % (
            before)
        # print("query=", query)
        cur.execute(query)
        con.commit()
...
def fetchOTH(timeDelta):
    try:
        ...
        before = now - timeDelta
        before = before.strftime('%Y-%m-%d %H:%M:%S')
        query = "SELECT OutdoorTemperature, OutdoorHumidity, TimeStamp FROM `WeatherRack3` WHERE (TimeStamp > '%s') ORDER BY id ASC" % (
            before)
        # print("query=", query)
        cur.execute(query)
        con.commit()
...
def fetchSUV(timeDelta):
    try:
        ...
        before = now - timeDelta
        before = before.strftime('%Y-%m-%d %H:%M:%S')
        query = "SELECT SunlightVisible,TimeStamp FROM `WeatherRack3` WHERE (TimeStamp > '%s') ORDER BY id ASC" % (
            before)
        # print("query=", query)
        cur.execute(query)
        con.commit()
...
def buildSunlight_UVIndex_Graph():
    ...
    graph = dcc.Graph(
        id={'type': 'WR3PGdynamic', 'index': 'graph-suv'},
        figure=fig,
        animate=False
    )
    return graph
...
def WR3Page():
    ...
    print("WR3P-CWSJSON=", CWJSON)
    Row1 = html.Div(
        [
            # dbc.Row( dbc.Col(html.Div(id="WeatherRack3 Instruments"))),
            dbc.Row(dbc.Col(
                html.Div(html.H6(id={'type': 'WR3Pdynamic', 'index': "StringTime"}, children="Weather Instruments")))),
                ...
                [
                    dbc.Col(html.Div(
                        [
                            ...
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "OutdoorTemperature"},
                            ...
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "OutdoorHumidity"},
                                         children=str(round(CWJSON["OutdoorHumidity"], 1)) + " %",
                                         style={"font-size": maintextsize, "color": maintextcolor}),
                                 html.P("Outdoor Humidity", style={"color": subtextcolor})
                                 ], id="ot1", className="mini_container", ),
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "SunlightVisible"},
            ...
            dbc.Row(
                [
                    dbc.Col(html.Div(
                        [
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "TotalRain"},
                            ...
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "CalendarDayRain"},
                                         children=str(CWJSON["CalendarDayRain"]) + RUnits(),
                                         style={"font-size": maintextsize, "color": maintextcolor}),
                                 html.P("Daily Rain", style={"color": subtextcolor})
                                 ], id="ot1", className="mini_container", ),
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "CalendarMonthRain"},
                            ...
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "30DayRain"},
                    ...
                    dbc.Col(html.Div(
                        [
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "24HourRain"},
                            ...
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "7DaysRain"},
                            ...
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "AQI"},
                            ...
                            html.Div(
                                [html.H1(id={'type': 'WR3Pdynamic', 'index': "AQI24Average"},
